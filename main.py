...
def main() -> None:
    ...
    # 2. 抓取 RSS Feed
    logger.info("步驟 2/4：抓取 RSS Feed")
    articles = fetch_articles()

    if not articles:
        logger.warning("今日沒有抓到任何新聞，仍會發送通知")

    # 3. Claude API 整理摘要
    logger.info("步驟 3/4：使用 Claude API 整理摘要")
    summary = process_news(articles, date_str)

    # 4. LINE Push Message 發送
    logger.info("步驟 4/4：透過 LINE 發送訊息")
    send_message(summary)

    logger.info("=== 每日 AI 新聞彙整完成 ===")
